...
void nesapu_vt_device::vt_apu_write(uint8_t address, uint8_t data) {
	...
	if(address == 0x35 && !m_apu_vt.use_vt3x_pcm)
	{
		...
		m_xop2->write(machine().dummy_space(), 0x15, data & 0x0F);
		...
		nesapu_device::write(machine().dummy_space(), 0x15, nes_val);
	}
...
uint8_t nesapu_vt_device::vt_apu_read(uint8_t address) {
	if(address >= 0x30 && address <= 0x36)
	{
		...
		uint8_t base = nesapu_device::read(machine().dummy_space(), 0x15);
...
READ8_MEMBER( nesapu_vt_device::read )
{
	if (offset <= 0x0F)
	{
		return nesapu_device::read(space, offset);
	} else if (offset >= 0x10 && offset <= 0x13) {
		if(!m_apu_vt.use_vt03_pcm)
			return nesapu_device::read(space, offset);
		else
			return vt_apu_read(offset);
	} else if (offset >= 0x20 && offset <= 0x2F) {
		return m_xop2->read(space, offset - 0x20);
	} else if (offset == 0x15 || (offset >= 0x30 && offset <= 0x36)) {
		return vt_apu_read(offset);
	} else {
...
WRITE8_MEMBER( nesapu_vt_device::write )
{
	if (offset <= 0x0F)
	{
		nesapu_device::write(space, offset, data);
	} else if (offset >= 0x10 && offset <= 0x13) {
		//PCM registers affect both new and legacy APU
		if (m_apu_vt.use_vt03_pcm || m_apu_vt.use_vt3x_pcm)
			vt_apu_write(offset, data);
		else
			nesapu_device::write(space, offset, data);
	} else if (offset >= 0x20 && offset <= 0x2F) {
		m_xop2->write(space, offset - 0x20, data);
	} else if (offset == 0x15 || (offset >= 0x30 && offset <= 0x36)) {
		vt_apu_write(offset, data);
	} else {
