...
#define MCFG_NES_VT_APU_ROM_READ_CALLBACK(_devcb) \
	devcb = &nesapu_vt_device::set_rom_read_callback(*device, DEVCB_##_devcb);
...
public:
	...
	template <class Object> static devcb_base &set_rom_read_callback(device_t &device, Object &&cb) { return downcast<nesapu_vt_device &>(device).m_rom_read_cb.set_callback(std::forward<Object>(cb)); }
